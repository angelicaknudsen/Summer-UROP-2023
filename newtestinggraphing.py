...
#make sure file is in same folder as this program
file = input("Enter the name of a file (including relative path) to graph. Make sure it ends with an appropriate suffix (e.g. .csv, .xls, or .xlsx): ")

print("Loading graph for: " + file)
#file = r"Summer-UROP-2023\HANNA_6-13-23_16.32.xls"
#file = Summer-UROP-2023\MANTA_06-12-2023_17.04.csv
#file = r"Summer-UROP-2023\CASTAWAY_06-13-2023_15.04.csv"
...
def excel_to_csv(oldfile, newfile):
    if "HANNA" in file.upper():
        sheet = pd.read_excel(oldfile, sheet_name=1 , index_col=None)
...
#title data of Hanna is in first row, so sifting through file to clean it up is not necessary
...
#determining which column contains independent variables and which file type needs to have values converted to floats; used to reference other ones
...
for j in range(0, len(mega_list[0])):
    ...
    for k in range (1, len(mega_list)): #starting at one since title is in first row, which we don't want graphed
        if convert_to_float:
            x.append(float(mega_list[k][independent_index]))
            y.append(float(mega_list[k][j]))
        else:
